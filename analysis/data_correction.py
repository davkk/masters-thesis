...
if __name__ == "__main__":
    ...
    for idx, (cf_truth, cf_nocor, cf_cor_1d, cf_cor_2d) in enumerate(
        ...
        X_ref = cf_nocor
        mask = [X_ref.phi_mask, X_ref.eta_mask][idx]
        X = [X_ref.phi[mask], X_ref.eta[mask]]
        ...
        if args.mc:
            ...
            rel_err_cor_1d = cf_cor_1d.err[mask] / np.maximum(
                cf_cor_1d.val[mask], 1e-10
            )
            rel_err_truth_1d = cf_truth.err[mask] / np.maximum(
                cf_truth.val[mask], 1e-10
            )
            ...
            rel_err_cor_2d = cf_cor_2d.err[mask] / np.maximum(
                cf_cor_2d.val[mask], 1e-10
            )
            rel_err_truth_2d = cf_truth.err[mask] / np.maximum(
                cf_truth.val[mask], 1e-10
            )
