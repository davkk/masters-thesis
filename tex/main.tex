...
labore et dolore magnam aliquam quaerat voluptatem. Ut enim aeque doleamus animo, cum
corpore dolemus, fieri tamen permagna accessio potest, si aliquod aeternum et infinitum
impendere.
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.8\textwidth]{img/detector-angles.png}
    \caption{Showcase of the angles considered in angular correlation function analysis.}
    \label{fig:detector-angles}
\end{figure}

First, the pseudorapidity, $\eta$, relates to the angle between the particle momentum $p$ and the beam axis ($\theta$, fig. \ref{fig:detector-angles}) as:
\begin{equation}
    \eta = -\ln\left[\tan\left(\frac{\theta}{2}\right)\right].
\end{equation}

The azimuthal angle on the other hand represents the angle between the $x$-axis and the projection, $p_T$, of the momentum vector onto the $xy$-plane ($\varphi$, fig. \ref{fig:detector-angles}).
...
The initial ideas for the correction procedure proved unusable at such large scale. I changed the workflow direction to accommodate the Grid's parallel nature. Unfortunately, I did not achieve full automation, but I have integrated key features that allow for flexibility (fig. \ref{fig:workflow-temp}).

\begin{figure}[ht!]
    \centering
    \includegraphics[width=\textwidth]{img/workflow-temp.png}
    \caption{Visualization of the next workflow idea for efficiency correction.}
    \label{fig:workflow-temp}
\end{figure}
...
When the user specifies a projection through a flag, the macro calls ROOT's \verb|Project3D()| method to collapse the third axis into a 1D or 2D distribution (lst. \ref{lst:corr-macro-proj}).
...
\begin{figure}[ht!]
    \centering
    \caption{A comparison of the reconstruction efficiency (a) and the contamination factor (b) for different particle types.}
    \includegraphics[width=\textwidth]{../data/LHC24f3c/effcor/eff-cont.pdf}
\end{figure}
